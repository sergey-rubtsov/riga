...
public class SparkAnalysis {
    ...
    private static void processAllEvents(SQLContext sqlContext) {
        ...
        Dataset<Row> vehiclesOnRoute = sqlContext
                .sql("SELECT first(Virziens) as direction, first(TMarsruts) as route, GarNr, vehicles.VehicleID, min(time) as first_time, max(time) as last_time, count(time) as events FROM validation_events " +
        ...
        transportEvents.createOrReplaceTempView("transport_events");
        transportEvents = sqlContext
                .sql("SELECT VehicleID, GarNr, TMarsruts, route, Virziens as direction, Code, WGS84Fi, WGS84La, " +
                        "VehicleMessageID as vehicle_message_id, event_source, ValidTalonaId, timestamp, timestamp_of_transport_event " +
                        "FROM transport_events")
                .withColumn("route", coalesce(col("TMarsruts"), col("route")))
                .drop("TMarsruts");
        transportEvents.show();
        validationEvents.show();


//        Dataset<Row> actualStopsAndEvents = transportEvents.groupBy(
...
//        ).count();
        ...
        Dataset<Row> actualStopsAndRealStops = transportEvents
                .join(schedule, new Set.Set2<>("route", "direction").toSeq(), "left");
        ...
        actualStopsAndRealStops = actualStopsAndRealStops.withColumn("diff",
                abs(actualStopsAndRealStops.col("stop_lat")
                        .$minus(actualStopsAndRealStops.col("WGS84Fi"))).$times(coefficient).
                        plus(abs(actualStopsAndRealStops.col("stop_lon")
                                .$minus(actualStopsAndRealStops.col("WGS84La")))));
