...
public class BatchConfig {
    ...
    @Bean
    @StepScope
    public ItemReader<Trip> tripItemReader() {
    ...
    @Bean
    public ItemWriter<Trip> tripItemWriter() {
        return new TripItemWriter();
    }

    @Bean
    public LineMapper<Trip> tripLineMapper() {
        DefaultLineMapper<Trip> defaultLineMapper = new DefaultLineMapper<>();
        DelimitedLineTokenizer tokenizer = new DelimitedLineTokenizer();
        tokenizer.setStrict(strict);
        defaultLineMapper.setLineTokenizer(tokenizer);
        FieldSetMapper<Trip> fieldSetMapper = new TripFieldSetMapper();
        defaultLineMapper.setFieldSetMapper(fieldSetMapper);
        return defaultLineMapper;
    }

    @Bean
    public Step tripStep(@Qualifier("tripItemReader") ItemReader<Trip> reader,
                      @Qualifier("tripItemWriter") ItemWriter<Trip> writer) {
        return stepBuilderFactory.get("trip")
                .<Trip, Trip> chunk(CHUNK_SIZE)
                .reader(reader)
